...
TLS 1.2 的十个 ARIA-GCM 套件采用真实 ARIA-128/256 与 GCM 记录保护，仅在入站 ClientHello 实际提供对应套件时映射到 B→A。独立 OpenSSL 对端覆盖 RSA、DHE-RSA、DHE-DSS、ECDHE-RSA 与 ECDHE-ECDSA 的成功握手和大于 16 KiB 的加密 HTTP，并验证密文/认证标签篡改及未提供套件均被拒绝。实现限制为 TLS 1.2 GCM，不把 ARIA 标识伪装成 AES，也不宣称具备 TLS 1.3、CBC 或 DTLS ARIA 能力。实现边界、双架构负例和 72 项矩阵结果见 [ARIA 验证记录](test-results/ARIA-VALIDATION.md)。

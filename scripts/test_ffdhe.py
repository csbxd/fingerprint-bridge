    ...
    def test_rfc7919_negotiation_hrr_and_illegal_public_keys(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for group_id, bits in enumerate((2048, 3072, 4096, 6144, 8192), 256):
                ...
                cases = [('1_2', 'valid'), ('1_3', 'valid'), ('1_3', 'zero'),
                         ('1_3', 'one'), ('1_3', 'too-large'), ('1_3', 'unoffered')]
                if bits > 4096:
                    cases.append(('1_2', 'unoffered'))
                    ...
                    with self.subTest(group=group, tls=version, mode=mode):
                        ...
                        try:
                            ...
                            client = subprocess.run([
                                ...
                                # OpenSSL omits FFDHE from supported_groups when
                                # forced TLS-1.2-only. Offer its normal versions;
                                # the controlled origin selects TLS 1.2 instead.
                                *(['-tls1_3'] if version == '1_3' else []),
                                '-groups', groups, '-quiet'
                            ...
                            if mode == 'valid':
                                ...
                                incoming, = (case / 'runtime').glob('*.inbound.json')
                                offered = json.loads(incoming.read_text())['tls']['fields']['groups']
                                self.assertIn(group_id, offered, diagnostic)
                                self.assertEqual(fields['groups'], offered, diagnostic)
                                if version == '1_3':
